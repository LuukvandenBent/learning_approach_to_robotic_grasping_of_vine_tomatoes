...
class autoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        with amp.autocast(enabled=p.device.type != 'cpu'):
            ...
            # Post-process
            y = non_max_suppression(y, conf_thres=self.conf, iou_thres=self.iou, classes=self.classes, kpt_label=(self.model.yaml['nc']>0), nc=self.model.yaml['nc'], nkpt=self.model.yaml['nkpt'])  # NMS
            #for i in range(n):
