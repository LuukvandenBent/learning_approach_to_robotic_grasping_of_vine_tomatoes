...
class DetermineGraspCandidatesKeypoints():
    def __init__(self, NODE_NAME):
        ...
        self.find_grasp_candidates_keypoints_services = rospy.Service('find_grasp_candidates_keypoints', find_grasp_candidates_command, self.execute_command)
    ...
    def execute_command(self, map):
        ...
        try:
            grasp_candidates = self.determine_grasp_candidates_keypoints(pointcloud=map.map)
            return grasp_candidates
        except Exception as e:
            print(e)
            return None
    
    def determine_grasp_candidates_keypoints(self, pointcloud):
        ...
        #invert the rescale for the bbox
            bbox[[0,1,2,3,6,7,9,10,12,13]] *= image_scale
            bbox[[1,3,7,10,13]] -= pad_size
    ...
    def generate_grasp_pose(self, projection_history, bbox, numpy_x, numpy_y, numpy_z):
        ...
        keypoint_left = (int(bbox[9]), int(bbox[10]))
        keypoint_right = (int(bbox[12]), int(bbox[13]))
    ...
    def publish_debug_image(self, debug_image, bboxes):
        ...
        for i in range(bboxes.shape[0]):
            ...
            x_c,y_c,x_l,y_l,x_r,y_r = int(bbox[6]), int(bbox[7]), int(bbox[9]), int(bbox[10]), int(bbox[12]), int(bbox[13])
